...
export class OAuthAuth implements AuthStrategy {
  ...
  attach(axiosInstance: AxiosInstance): void {
    axiosInstance.interceptors.request.use((config) => {
      ...
      // MediaWikiClient.call() sends POST bodies as URLSearchParams, which oauth-1.0a can't read via plain
      // object enumeration — convert to a record so the signature covers the actual body params.
      const data =
        method === "GET"
