...
export const restRequest = async <T>(client: MediaWikiClient, config: AxiosRequestConfig): Promise<T> => {
  ...
  try {
    ...
    if (axios.isAxiosError(error) && error.response) {
      ...
      throw new MediaWikiApiError(body.message ?? body.detail ?? body.title ?? error.message, {
        code: String(body.httpCode ?? error.response.status),
 ...
 * the REST API requires for subpage titles (e.g. `"User:Foo/Bar"`).
